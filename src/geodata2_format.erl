...
make_meta(Data, Meta) ->
    ...
    V4Start = v4_tree_start(Data, MetaRec),
...
lookup_pos(#meta{node_count = NodeCount}, _, _Bits, NodeCount) ->
    not_found;
lookup_pos(#meta{node_count = NodeCount, tree_size = TreeSize} = Meta, Data, _, Pointer)
    when Pointer > NodeCount ->
    Offset = Pointer + TreeSize - NodeCount,
    {ok, parse_ptr({Meta, Data}, Offset)}.

lr(0, Offset, Whole, Remdr, RecordSize, Data) ->
    <<_:Offset, LeftL:Whole/integer, LeftH:Remdr/integer, _Right:RecordSize, _/binary>> =
        Data,
...
parse(_MD, <<?GEO_DOUBLE:3, 8:5, F:64/float, Rest/binary>>) ->
    {F, Rest};
parse({#meta{data_start = DS}, _} = MD,
      <<?GEO_POINTER:3, 0:2, P:11/unsigned-integer, Rest/binary>>) ->
    {parse_ptr(MD, P + DS), Rest};
parse({#meta{data_start = DS}, _} = MD,
      <<?GEO_POINTER:3, 1:2, P:19/unsigned-integer, Rest/binary>>) ->
    {parse_ptr(MD, P + 2048 + DS), Rest};
parse({#meta{data_start = DS}, _} = MD,
      <<?GEO_POINTER:3, 2:2, P:27/unsigned-integer, Rest/binary>>) ->
    {parse_ptr(MD, P + 526336 + DS), Rest};
parse({#meta{data_start = DS}, _} = MD,
      <<?GEO_POINTER:3, 3:2, _VVV:3, P:32, Rest/binary>>) ->
    {parse_ptr(MD, P + DS), Rest};
