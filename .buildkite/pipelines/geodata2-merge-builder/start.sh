...
EOF
  ...
  results=$(curl \
    ...
    "https://api.github.com/repos/SemanticSugar/geodata2/pulls?per_page=100&state=open" \
  ...
  for result in ${results}; do
    ...
    builds=$(curl \
      ...
      -H "Authorization: token ${GITHUB_TOKEN}" \
      ${status} \
      | jq '.[] | select(.target_url | contains("geodata2-merge-builder")) | [.target_url] | @tsv' \
    )
    latest_build=0
...
cat <<EOF
    - "$(bk_url pipelines/geodata2-merge-builder/builds/${latest_build}/rebuild)"
EOF
    fi
  done
else
  # Read the pipeline and output replacing variables
  sed -e "s/\$MERGE_BRANCH/master/g" ${root}/pipelines/geodata2-pr-builder/start.yml
fi
